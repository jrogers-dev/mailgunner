class MailController < ApplicationController
  ...
  #Future use would be to look up all queued payloads and 
  #view whether they were successful or not
  def show
  ...
  #Once we've established the metadata looks good, we send the payload down the queue pipeline
  #ActionJob > Sidekiq > Redis 
  def queue_payload()
    begin
      SendMailJob.perform_later(@payload)
    rescue => e
      @response = e.message
    else
      @response = {message: "Email payload queued and will be sent shortly.", response: 200}
    end
  end
end